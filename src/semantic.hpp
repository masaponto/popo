            ...
            -> std::unique_ptr<syntax::cons_node>
        {
            ...
            switch (value_cons_node->car->type) {
                case syntax::node_type::cons:
                {
                    ...
                    //                     std::make_pair(
                    //                         pair.first,
                    //                         std::unique_ptr<symbol_table_entry>(new
                    // function_entry(pair.second))));
...
//             std::cout << "check cons func_name: "<< function_name << std::endl;
            ...
            std::unique_ptr<syntax::cons_node> ret_cons;
            switch (divide_function(function_name)) {
                case function_type::DEFINE:
                    ret_cons = define_procedure(std::move(cons->cdr));
                    break;
                //                     return nullptr;

                case function_type::LAMBDA:
                    ret_cons = lambda_procedure(std::move(cons->cdr));
                    break;
